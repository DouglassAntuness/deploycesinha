app.controller("integracaoFacebook", function ($scope, $http, $rootScope) {
    ...
    g$.gerarTemplateFB = function (params) {
        var params = g$.alterSargentos(params),
            produto = params[1],
            template = params[2].trim(),
            destino = $("[data-id=13544]")[0];
        if (!template) { return Materialize.toast("Você deve Selecionar um Template", 4000, 'red darken-1'); }
        if (!produto) { return Materialize.toast("Você deve Selecionar um Produto", 4000, 'red darken-1'); }
        var query = "SELECT * FROM " + $rootScope.user.banco + ".produto where id = " + produto
        $http.get("/get/" + query).then(function (data) {
            ...
            destino.innerHTML = "<div><img id='cnvs' src=''></div>"
        ...
        //img_in1.src = '../img/novoTemplate.jpg'
        });
    }
